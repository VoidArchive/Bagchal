import type { GameState, Point, Line, PieceType, CaptureInfo } from './types';
...
export function findPointByCoords(points: Point[], x: number, y: number): Point | undefined {
...
export function calculateValidTigerMoves(
	...
	const tigerPoint = points.find((p) => p.id === tigerId); // Find tiger's coordinates
	if (!tigerPoint) return { destinations, captures }; // Should not happen
	...
	neighbors.forEach((adjId) => {
		...
		if (piece === null) {
			...
			const adjPoint = points.find((p) => p.id === adjId); // Find goat's coordinates
			if (!adjPoint) return; // Should not happen
			...
			const destPoint = findPointByCoords(points, destX, destY);

			// Check if the destination point exists, is empty, AND is actually adjacent to the goat
			// (This last check ensures it's a valid line segment jump)
			if (
				destPoint &&
...
export function checkIfTigersAreTrapped(
	...
	state.message = '--- Checking if tigers are trapped ---'; // Explicit log start
	...
	for (const tigerId of tigerIds) {
		...
		// Ensure calculateValidTigerMoves is correctly called
		...
		if (destinations.length > 0) {
			...
			return false; // Found a movable tiger
		}
	}
	state.message =
		'NO tigers found with valid moves. Returning true (trapped). \n --- Trap check finished ---';
	return true; // All tigers checked, none could move
}
...
export function executeMove(
	...
	// Need adjacency and points again for the check
	adjacency: Map<number, number[]>,
	points: Point[]
): void {
	const movingPiece = state.board[fromId]; // Store piece type *before* moving
	...
	// 3. Check GOAT win by trapping *immediately after* a GOAT move, if no winner yet
...
// Reset game to initial state using global $state
